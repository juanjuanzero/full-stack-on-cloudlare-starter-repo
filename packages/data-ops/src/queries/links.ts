...
export async function getLink(linkId: string) {
    ...
    if (!parsedLink.success) {
        ...
        throw new Error("BAD_REQUEST Error Parsing Link Data");
    }
    return parsedLink.data;
}

export async function updateLinkDestinations(linkId: string, destination: DestinationsSchemaType) {
    const desitnationParse = destinationsSchema.parse(destination);
    const db = getDb();
    await db.update(links).set({
        destinations: JSON.stringify(desitnationParse),
        updated: new Date().toISOString(),
    }).where(eq(links.linkId, linkId));
}